...
const progressBar = (followers, currentFollowers, users, count, next_max_id) => {
    const percentage = Math.round((followers.length / currentFollowers) * 10)
    console.log(`+ ${count} from ${users.length}, (${followers.length}), [${'#'.repeat(percentage)}${' '.repeat(10 - percentage)}] MaxID: ${next_max_id}`)
}
    ...
    try {
        ...
        while (followers.length < currentFollowers) {
            ...
            if (next_max_id == undefined) {
                ...
                users.forEach(p => console.log(p.username, p.full_name))
                console.log(linkMid)
                return
                users.forEach(follower => {
                    if (!followers.find(f => f.username === follower.username)) {
